...
class HMM():
	...
	def cal_gconst(self):
		for state,mixnum in list(self.weight.keys()):
	...
	def b(self,state,x):
		result = {}
		for a,b in list(self.weight.keys()):	#iterate g
			if a != state: continue
			result[b] = 0.0
...
def build_unit_hmm(hmmlist):
	for name in range(len(hmm_header_my.phones)):
		sound=hmm_header_my.phones[name][0]
		numstates=len(hmm_header_my.phones[name][1])
	
		hmmlist[sound]=HMM(numstates)

		for i in range(numstates):
			hmmlist[sound].a[i]=hmm_header_my.phones[name][1][i]

		for state in range(len(hmm_header_my.phones[name][2])):
			for pdf in range(N_PDF):
				hmmlist[sound].weight[state+1,pdf+1]=hmm_header_my.phones[name][2][state][pdf][0]
				hmmlist[sound].mean[state+1,pdf+1]=hmm_header_my.phones[name][2][state][pdf][1]
				hmmlist[sound].variance[state+1,pdf+1]=hmm_header_my.phones[name][2][state][pdf][2]
...
for line in bi:
	temp = line.split()
	bigram[temp[0],temp[1]] = float(temp[2])*12
...
for line in dic:
	...
	if temp[0] != "<s>":
		...
		if temp[0] == "zero" and temp[2] == "iy":
			words["zero2"] = pho = ['sil'] + temp[1:]
		else:
			words[temp[0]] = pho = ['sil'] + temp[1:]


print("3. reading unigram")
for line in uni:
	temp = line.split()
	if temp[0] != "<s>":
		unigram[temp[0]] = float(temp[1])*(100/9)

uni.close()
dic.close()

#read hmm_header and build unit HMMs
print("4. building Unit HMM")
build_unit_hmm(hmmlist)

#calculate gconst value
for hmm in list(hmmlist.keys()):	
...
for word_row in hmm_index:
	...
	for word_col in hmm_index:
		...
		hmm.a[b-1][c]=0.0
		hmm.a[b][c]=0.0
		if not ((word_row == 'oh' and word_col == 'zero') or (word_col == 'oh' and word_row == 'zero') or (word_col == 'oh' and word_row == 'zero2')
		 or (word_row == 'oh' and word_col == 'zero2') or (word_col == 'zero' and word_row == 'zero2') or (word_col == 'zero2' and word_row == 'zero')): 
